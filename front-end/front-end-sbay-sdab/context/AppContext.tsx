...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const analyzeUrl = useCallback(async (url: string) => {
    ...
    try {
      ...
      const infoData = res.data as Record<string, unknown> | null;
      if (infoData?._fallback && infoData?._error) {
        showToastRef.current('warning', 'Limited Results', 'Fetch failed: ' + (infoData._error as string).substring(0, 300));
      }
      dispatch({ type: 'SET_CURRENT_VIDEO_INFO', payload: { ...(infoData || {}), sourceUrl: url } });
